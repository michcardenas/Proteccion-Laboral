...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div>
                <h1 class="text-lg font-semibold text-slate-900 sm:text-xl">Contratos</h1>
                <p class="text-xs text-slate-500">Acuerdos vigentes y pasados con los clientes.</p>
            </div>
        </template>
        ...
        <div class="space-y-5">
            ...
            <section class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-xs font-medium uppercase tracking-wider text-slate-500">Total contratos</p>
                    <p class="mt-2 text-3xl font-semibold tracking-tight text-slate-900">{{ totals.count }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-xs font-medium uppercase tracking-wider text-slate-500">Activos</p>
                    <p class="mt-2 text-3xl font-semibold tracking-tight text-emerald-700">{{ totals.activos }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-xs font-medium uppercase tracking-wider text-slate-500">Valor en activos</p>
                    <p class="mt-2 text-2xl font-semibold tracking-tight text-slate-900">{{ formatCurrency(totals.valor_activos) }}</p>
                </div>
            </section>
            ...
            >
                <template #cell-codigo="{ row }">
                    ...
                    <p v-if="row.processes_count" class="text-[10px] text-slate-500">
                ...
                <template #cell-client="{ row }">
                    <Link
                        ...
                        class="text-sm text-slate-700 hover:text-brand-900"
                    >
                        {{ row.client.razon_social }}
                    </Link>
                    <span v-else class="text-xs text-slate-400">—</span>
                    <p v-if="row.client?.nit" class="text-xs text-slate-500">NIT {{ row.client.nit }}</p>
                </template>

                <template #cell-service="{ row }">
                    <p class="text-sm text-slate-700">{{ row.service?.nombre || '—' }}</p>
                    <StatusBadge
                        v-if="row.service"
                ...
                <template #cell-fechas="{ row }">
                    <p class="text-xs text-slate-700">{{ formatDate(row.fecha_inicio) || '—' }}</p>
                    <p class="text-xs text-slate-500">→ {{ formatDate(row.fecha_fin) || 'Indefinido' }}</p>
                </template>

                <template #cell-valor="{ row }">
                    <p class="font-medium text-slate-900">{{ formatCurrency(row.valor) }}</p>
                    <p class="text-xs text-slate-500">{{ row.modalidad_pago }}</p>
                </template>
            </DataTable>
        </div>
    </AuthenticatedLayout>
