    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-2">
                ...
                <PageHeader :titulo="'Contrato ' + contract.codigo" :resumen="(contract.client?.razon_social || 'Sin cliente') + (contract.service ? ' · ' + contract.service.nombre : '')" help-key="contracts" />
            </div>
        </template>
        ...
        <div class="space-y-5">
            ...
            <section class="rounded-2xl border border-brand-200 bg-white p-6 shadow-sm">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <div class="flex flex-wrap items-center gap-2">
                            <h2 class="text-xl font-semibold text-brand-900">{{ contract.codigo }}</h2>
                ...
                <dl class="mt-6 grid grid-cols-2 gap-4 border-t border-brand-100 pt-6 lg:grid-cols-5">
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-brand-500">Servicio</dt>
                        <dd class="mt-1 text-sm text-brand-900">{{ contract.service?.nombre || '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-brand-500">Inicio</dt>
                        <dd class="mt-1 text-sm text-brand-900">{{ formatDate(contract.fecha_inicio) }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-brand-500">Fin</dt>
                        <dd class="mt-1 text-sm text-brand-900">{{ contract.fecha_fin ? formatDate(contract.fecha_fin) : 'Indefinido' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-brand-500">Valor</dt>
                        <dd class="mt-1 text-sm font-semibold text-brand-900">{{ formatCurrency(contract.valor) }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-brand-500">Modalidad pago</dt>
                        <dd class="mt-1 text-sm text-brand-900">{{ contract.modalidad_pago }}</dd>
                    </div>
                </dl>
            </section>
            ...
            <section v-if="activeTab === 'resumen'" class="grid grid-cols-1 gap-5 lg:grid-cols-3">
                <div class="rounded-2xl border border-brand-200 bg-white p-6 shadow-sm lg:col-span-2">
                    <h3 class="text-sm font-semibold uppercase tracking-wider text-brand-500">Detalle del servicio</h3>
                    <p class="mt-3 text-sm text-brand-700">
                        {{ contract.service?.descripcion || 'Sin descripción del servicio.' }}
                    </p>
                    <h3 class="mt-6 text-sm font-semibold uppercase tracking-wider text-brand-500">Notas internas</h3>
                ...
                <div class="space-y-4">
                    <div class="rounded-2xl border border-brand-200 bg-white p-5 shadow-sm">
                        <p class="text-xs font-medium uppercase tracking-wider text-brand-500">Total facturado</p>
                        <p class="mt-2 text-2xl font-semibold text-brand-900">{{ formatCurrency(totalInvoiced) }}</p>
                    </div>
                    <div class="rounded-2xl border border-brand-200 bg-white p-5 shadow-sm">
                        <p class="text-xs font-medium uppercase tracking-wider text-brand-500">Pagado</p>
                        <p class="mt-2 text-2xl font-semibold text-success-700">{{ formatCurrency(totalPaid) }}</p>
                    </div>
                    <div class="rounded-2xl border border-brand-200 bg-white p-5 shadow-sm">
                        <p class="text-xs font-medium uppercase tracking-wider text-brand-500">Cliente</p>
                        <p class="mt-1 text-sm font-medium text-brand-900">{{ contract.client?.razon_social || '—' }}</p>
                        <p class="text-xs text-brand-500">{{ contract.client?.email || 'Sin email' }}</p>
                        <p class="text-xs text-brand-500">{{ contract.client?.telefono || 'Sin teléfono' }}</p>
                    </div>
                </div>
            </section>

            <!-- Procesos -->
            <section v-else-if="activeTab === 'procesos'" class="overflow-hidden rounded-xl border border-brand-200 bg-white shadow-sm">
                <ul class="divide-y divide-brand-100">
                    <li v-if="!contract.processes.length" class="px-6 py-8 text-center text-sm text-brand-500">
            ...
            <!-- Facturas -->
            <section v-else-if="activeTab === 'facturas'" class="overflow-hidden rounded-xl border border-brand-200 bg-white shadow-sm">
                <ul class="divide-y divide-brand-100">
                    <li v-if="!contract.invoices.length" class="px-6 py-8 text-center text-sm text-brand-500">
                    ...
                    >
                        <div class="min-w-0">
                            <div class="flex items-center gap-2">
                                <p class="text-sm font-medium text-brand-900">{{ i.numero }}</p>
                        ...
                        <p class="text-sm font-semibold text-brand-900">{{ formatCurrency(i.total) }}</p>
                    </li>
                </ul>
            </section>
        </div>
