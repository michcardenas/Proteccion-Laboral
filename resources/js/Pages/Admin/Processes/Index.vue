...
// El ESTADO sí lleva color semántico: es lo que hay que poder escanear de un vistazo.
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <PageHeader titulo="Procesos" help-key="processes" />
        </template>
        ...
        <div class="space-y-5">
            ...
            <section class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div class="rounded-xl border border-brand-200 bg-white p-5 shadow-sm">
                    <p class="text-xs font-medium uppercase tracking-wider text-brand-500">Total procesos</p>
                    <p class="mt-2 text-3xl font-semibold tracking-tight text-brand-900">{{ totals.count }}</p>
                </div>
                <div class="rounded-xl border border-brand-200 bg-white p-5 shadow-sm">
                    <p class="text-xs font-medium uppercase tracking-wider text-brand-500">Abiertos / en curso</p>
                    <p class="mt-2 text-3xl font-semibold tracking-tight text-accent-700">{{ totals.abiertos }}</p>
                </div>
                <div class="rounded-xl border border-brand-200 bg-white p-5 shadow-sm">
                    <p class="text-xs font-medium uppercase tracking-wider text-brand-500">Cerrados / archivados</p>
                    <p class="mt-2 text-3xl font-semibold tracking-tight text-success-700">{{ totals.cerrados }}</p>
                </div>
            </section>
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="flex gap-2">
                        <select v-model="estado" class="flex-1 rounded-md border-brand-300 text-sm shadow-sm focus:border-brand-900 focus:ring-brand-900">
                            ...
                            <option v-for="e in estados" :key="e" :value="e">{{ etiqueta('estadoProceso', e) }}</option>
                        </select>
                        <button
            ...
            >
                <template #cell-codigo="{ row }">
                    ...
                    <p class="text-[10px] text-brand-500">Abierto {{ formatDate(row.fecha_apertura) }}</p>
                </template>

                <template #cell-titulo="{ row }">
                    <p class="text-sm text-brand-700 line-clamp-2">{{ row.titulo }}</p>
                </template>

                <template #cell-client="{ row }">
                    <Link v-if="row.client" :href="route('admin.clients.show', row.client.id)" class="text-xs text-brand-700 hover:text-brand-900">
                ...
                <template #cell-service="{ row }">
                    <p class="text-xs text-brand-700">{{ row.service?.nombre || '—' }}</p>
                    <StatusBadge
                        v-if="row.service"
                        variant="neutral"
                        :label="etiqueta('modalidad', row.service.modalidad)"
                    />
                </template>

                <template #cell-estado="{ row }">
                    <StatusBadge :variant="estadoVariants[row.estado] || 'neutral'" :label="etiqueta('estadoProceso', row.estado)" />
                </template>

                <template #cell-lider="{ row }">
                    <p class="text-xs text-brand-700">{{ row.lider || '—' }}</p>
                    <p class="text-[10px] text-brand-500">Apoderado: {{ row.apoderado || '—' }}</p>
                </template>
                ...
                <template #cell-progress="{ row }">
                    <div class="flex flex-col items-end gap-1">
                        ...
                        <div class="h-1.5 w-24 overflow-hidden rounded-full bg-brand-100">
                            <div
