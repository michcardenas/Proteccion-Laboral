...
const navSections = computed(() => {
    ...
    if (hasRole('contador') || can('invoices.view') || can('accounting.view')) {
        sections.push({
            title: 'Finanzas',
            items: [
                { label: 'Facturación', href: '#', icon: icons.invoices, key: 'invoices', disabled: true, soon: true },
            ],
        });
    }
...
<template>
    <aside class="flex h-full w-72 flex-col border-r border-slate-200 bg-white">
        <!-- Brand -->
        <div class="flex items-center justify-between gap-2 border-b border-slate-100 px-5 py-5">
