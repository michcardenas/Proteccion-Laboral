    ...
    <Modal :show="show" max-width="4xl" @close="close">
        <div class="bg-white">
            ...
            <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
                <div>
                    <h3 class="text-lg font-semibold text-slate-900">Importar plan / contrato con IA</h3>
            ...
            <div class="max-h-[72vh] space-y-5 overflow-y-auto px-6 py-5">
                ...
                <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
                    ...
                    <p class="mb-2 mt-0.5 text-xs text-slate-500">Formatos: PDF, DOCX o TXT (máx. 20 MB). La IA leerá el documento y propondrá las etapas, entregables y tareas.</p>
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <span v-if="analizando" class="text-sm text-slate-500">Interpretando con Claude (10–40s)…</span>
                    </div>
                </div>
                ...
                <div v-if="tieneResultado" class="space-y-6 border-t border-slate-100 pt-5">
                    <div class="flex items-start justify-between gap-4">
                        <p class="text-sm text-slate-600"><span class="font-medium text-slate-800">Resumen IA:</span> {{ extraccion.resumen || '—' }}</p>
                        <span class="shrink-0 rounded-md bg-teal-50 px-2 py-0.5 text-xs font-medium text-teal-700 ring-1 ring-inset ring-teal-200">
                    ...
                    <section>
                        <div class="mb-2 flex items-center justify-between">
                            <h4 class="text-sm font-semibold text-slate-800">Etapas del plan ({{ extraccion.etapas.length }})</h4>
                            <button type="button" @click="addEtapa" class="text-xs font-medium text-indigo-600 hover:text-indigo-800">+ Añadir etapa</button>
                        </div>
                        <div class="space-y-4">
                            <div v-for="(etapa, i) in extraccion.etapas" :key="'e'+i" class="rounded-lg border border-slate-200 p-4">
                                ...
                                <div class="mt-3">
                                    <div class="mb-1 flex items-center justify-between">
                                        ...
                                        <button type="button" @click="addEntregable(etapa)" class="text-xs font-medium text-indigo-600 hover:text-indigo-800">+ Añadir entregable</button>
                                    </div>
                                    <div class="space-y-2">
                                        <div v-for="(ent, j) in etapa.entregables" :key="'ent'+i+'-'+j" class="flex items-center gap-2">
                                            ...
                                            <button type="button" @click="removeEntregable(etapa, j)" class="shrink-0 text-slate-400 hover:text-red-600" title="Quitar">&times;</button>
                                        </div>
                                        <p v-if="!etapa.entregables.length" class="text-xs text-slate-400">Sin entregables.</p>
                                    </div>
                                </div>

                                <div class="mt-3 text-right">
                                    <button type="button" @click="removeEtapa(i)" class="text-xs font-medium text-red-500 hover:text-red-700">Eliminar etapa</button>
                                </div>
                            </div>
                            <p v-if="!extraccion.etapas.length" class="text-sm text-slate-400">No se detectaron etapas. Puedes añadirlas manualmente.</p>
                        </div>
                    </section>
                    ...
                    <section>
                        <div class="mb-2 flex items-center justify-between">
                            <h4 class="text-sm font-semibold text-slate-800">Entregables transversales ({{ extraccion.transversales.length }})</h4>
                            <button type="button" @click="addTransversal" class="text-xs font-medium text-indigo-600 hover:text-indigo-800">+ Añadir</button>
                        </div>
                        <p class="mb-2 text-xs text-slate-500">Acompañamiento continuo durante todo el proceso (se enganchan a la primera etapa).</p>
                        <div class="space-y-2">
                            <div v-for="(t, i) in extraccion.transversales" :key="'tr'+i" class="flex items-center gap-2">
                                <input v-model="extraccion.transversales[i]" type="text"
                                    class="block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                <button type="button" @click="removeTransversal(i)" class="shrink-0 text-slate-400 hover:text-red-600" title="Quitar">&times;</button>
                            </div>
                            <p v-if="!extraccion.transversales.length" class="text-xs text-slate-400">Ninguno.</p>
                        </div>
                    </section>
                    ...
                    <section>
                        <div class="mb-2 flex items-center justify-between">
                            <h4 class="text-sm font-semibold text-slate-800">Tarjetas del tablero Kanban ({{ extraccion.tareas.length }})</h4>
                            <button type="button" @click="addTarea" class="text-xs font-medium text-indigo-600 hover:text-indigo-800">+ Añadir tarea</button>
                        </div>
                        <div class="space-y-3">
                            <div v-for="(tarea, i) in extraccion.tareas" :key="'t'+i" class="rounded-lg border border-slate-200 p-3">
                                <div class="grid grid-cols-1 gap-2 sm:grid-cols-12">
                                    <input v-model="tarea.titulo" type="text" maxlength="200" placeholder="Título de la tarea"
                                        class="sm:col-span-6 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                    <select v-model="tarea.prioridad"
                                        class="sm:col-span-3 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                        <option v-for="p in PRIORIDADES" :key="p" :value="p">{{ p }}</option>
                                    </select>
                                    <input v-model="tarea.fecha_limite" type="date"
                                        class="sm:col-span-2 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                    <button type="button" @click="removeTarea(i)" class="sm:col-span-1 text-slate-400 hover:text-red-600" title="Quitar">&times;</button>
                                </div>
                            </div>
                            <p v-if="!extraccion.tareas.length" class="text-sm text-slate-400">No se detectaron tareas. Puedes añadirlas manualmente.</p>
                        </div>
                    </section>
            ...
            <div class="flex items-center justify-between border-t border-slate-200 px-6 py-3">
                <span v-if="extraccion?.costo_usd != null" class="text-xs text-slate-400">Costo análisis: ${{ extraccion.costo_usd.toFixed(4) }} USD</span>
                <span v-else></span>
